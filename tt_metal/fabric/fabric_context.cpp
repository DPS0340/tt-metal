...
bool FabricContext::need_deadlock_avoidance_support(MeshId mesh_id, eth_chan_directions direction) const {
    ...
    if (topology_ == Topology::Torus) {
        ...
        TT_FATAL(
            ...
            mesh_id);
        const auto effective_fabric_type = get_fabric_type(fabric_config_, is_ubb_galaxy_, mesh_shape_it->second);
        if (direction == eth_chan_directions::NORTH || direction == eth_chan_directions::SOUTH) {
            return has_flag(effective_fabric_type, FabricType::TORUS_Y);
        }
        if (direction == eth_chan_directions::EAST || direction == eth_chan_directions::WEST) {
            return has_flag(effective_fabric_type, FabricType::TORUS_X);
        }
        // Z is an independent inter-mesh path, so keep deadlock avoidance for any configured torus.
        return direction == eth_chan_directions::Z &&
               get_fabric_type(fabric_config_, is_ubb_galaxy_) != FabricType::MESH;
    }
